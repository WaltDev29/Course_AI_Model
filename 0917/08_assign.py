...
x_train = x_train.reshape(12000,n_input)                # 32x32 영상을 1024차원 벡터로 펼침
x_test = x_test.reshape(2000,n_input)
...
def cross_validation(opt_list, learning_rate_list):
    opt_accuracy_list=[]

    for opt in opt_list:
        accuracy=[]
        for learning_rate in learning_rate_list:
            for train_index,val_index in KFold(n_splits=k, shuffle=True, random_state=42).split(x_train):   # 겹마다 훈련용/검증용 인덱스를 돌려준다.
                xtrain,xval=x_train[train_index],x_train[val_index]
                ytrain,yval=y_train[train_index],y_train[val_index]
                dmlp=build_model()      # 이전 겹의 학습 결과가 남지 않도록 매번 새로 만든다.
                dmlp.compile(loss='categorical_crossentropy',optimizer=opt(learning_rate=learning_rate),metrics=['accuracy'])   # opt() 로 새 옵티마이저 생성
        ...
        opt_accuracy_list.append(accuracy) # 각 옵티마이저별 정확률 5개가 담긴 리스트를 저장

    return opt_accuracy_list             # 각 옵티마이저의 정확률 5개가 담긴 리스트를 돌려준다.

# 옵티마이저 4개에 대해 교차 검증을 실행
# 옵티마이저 4개 x 5겹 = 모두 20번을 학습하므로 시간이 오래 걸린다.
# 교재 원본은 SGD() 처럼 괄호를 붙여 객체를 넘겼으나, 위 설명대로 이제는 괄호 없이 클래스 자체를 넘긴다.
opt_list = [SGD,Adam,Adagrad,RMSprop]
learning_rate_list = [0.01, 0.001, 0.0001, 0.00001]

acc_sgd, acc_adam, acc_adagrad, acc_rmsprop = cross_validation(opt_list, learning_rate_list)
